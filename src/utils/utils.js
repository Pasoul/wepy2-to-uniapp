 ...
 * 比如@click="handleClick"不是一个标准的w3c属性，所以无法解析
 * <view @click="handleClick" wx:if="{{true}}" :visible="true"><view> 转换成 <view a-_-aclick="handleClick" wx-_-wxif="{{true}}" dot-_-dotvisible="true"><view>
 * @param {*} fileText  wpy文件内容
 */
function replaceTagAndEventBind(fileText) {
  fileText = fileText
    .replace(/ @(.*?)="/g, ' a-_-a$1="')
    .replace(/ wx:(.*?)/g, " wx-_-wx$1")
    .replace(/ :(.*?)/g, " dot-_-dot$1");
  return fileText;
}
